...
private:
    ...
    bool is_rop(char op);                   // Проверяет, является ли символ ПРАВОСТОРОННЕЙ операцией
    ...
    bool moreq(char op1, char op2);         // Сравнивает приоритеты операций, нестрого
...
bool PolishRecordMaster::is_rop(char op)
{
    return (op == '^' || op == '_');
}
...
bool PolishRecordMaster::moreq(char op1, char op2)
{
    return (get_priority(op1) <= get_priority(op2));
}
