...
#include "cpp-bencoding-master/include/BDictionary.h"
#include "cpp-bencoding-master/include/BInteger.h"
#include "cpp-bencoding-master/include/BList.h"
#include "cpp-bencoding-master/include/BString.h"
#include "cpp-bencoding-master/include/Utils.h"
...
void PrettyPrinter::visit(BDictionary *bDictionary) {
    ...
    for (auto &item : *bDictionary) {
        ...
        if ((*item.first).value() == "info")
            this->pInfo = item.first;
        putComma = true;
    }
...
*/
std::pair<std::string, std::shared_ptr<BItem> > getPrettyRepr(std::shared_ptr<BItem> data,
        const std::string &indent) {
    auto prettyPrinter = PrettyPrinter::create();
    return std::pair<std::string, std::shared_ptr<BItem> >(prettyPrinter->getPrettyRepr(data, indent), prettyPrinter->pInfo);
}
