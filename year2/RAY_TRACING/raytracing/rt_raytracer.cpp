...
void RayTracer::readffGeometry(std::ifstream &f) {
    ...
    while (f) {
        ...
        else if (key == "quadrangle") {
            inPrimitive = 3;
            vxcounter = 0;
        }
        else if (key == "endquadrangle") {
            inPrimitive = 0;
            if (vxcounter == 4)
                scene.objects.push_back(new Quadrangle3d(quad));
        }
        else if (key == "vertex" && inPrimitive == 3) {
            ss >> x >> y >> z;
            quad[vxcounter % 4].set(x, y, z);
            ++vxcounter;
        }
        else if (key == "material" && inPrimitive == 3) {
            ss >> key;
            quad.setMaterialId(key);
        }
    }
}
